...
def panel_html(slug, app, server_port=None):
    ...
    parts = {
        "soundwave": (
                ...
                'if(k&&k.peaks)zl.textContent+=" \u00b7 green is the render";'
...
def dashboard():
    ...
    film_markup = (
        ...
        'src="' + app + '/projects/latest/video.mp4#t=${part_start}"></video>'
    ...
    film_script = (
        ...
        'if(!/^[a-f0-9]{12}$/.test(cid))cid="";'
        'if(!/^[a-f0-9]{16}$/.test(pid))pid="latest";'
        'var v=document.getElementById("ofilm"),lab=document.getElementById("ofilm-src");'
        'if(v){'
        'function show(id,how){var name=(id||"latest")+"-master.mp4";'
        'var url="%s/projects/"+pid+"/"+name+"#t="+ts;'
        'if(v.getAttribute("src")!==url)v.setAttribute("src",url);'
        'lab.textContent="render "+(id||"latest")+" \u00b7 replaced sound \u00b7 "+how;}'
        'show(cid,cid?"chosen":"latest");'
        'v.addEventListener("error",function(){'
        'var url="%s/projects/"+pid+"/video.mp4#t="+ts;'
        'if(v.getAttribute("src")!==url){v.setAttribute("src",url);'
        'lab.textContent="original recording \u00b7 no render available";}},{once:true});'
        '}'
    ) % (app, app)
    if hosted:
        add(
