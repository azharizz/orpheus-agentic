...
def panel_html(slug, app, server_port=None):
    ...
    parts = {
        "soundwave": (
                ...
                'function findcand(){if(cid)return Promise.resolve(cid);'
                'return ofetch(app+"/api/families?project_id="+pid,1)'
                '.then(function(r){return r.json();}).then(function(j){'
                'var fs=j.families||[];fs.forEach(function(f){'
                'if(f.latest_render_id)cid=f.latest_render_id;});return cid;})'
                ...
                '+(j.bin_duration_s||0).toFixed(3)+" s per bin"'
                ';'
                'draw();return findcand();}).then(function(){'
                'if(cid)zl.textContent+=" \u00b7 green is render "+cid;'
                'return getcand(zs,Math.min(ze,dur),900);}).then(function(k){'
