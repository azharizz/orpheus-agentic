...
- Backend suite: 84 tests pass, one opt-in live MCP test is skipped by default.
...
- Managed Session create/get/append round-trip now passes with required invocation IDs on internal state events. Memory Bank add/search also passes for the hosted project, and the application retrieval helper returns the stored preference.
...
- Agent Engine Session quota can still return `429`/`ServerError`; hosted dispatch and the worker have an explicit bounded Cloud Run/Cloud SQL fallback so a transient managed quota outage does not strand an approved run. The parity turn saw a transient Memory Bank write `ServerError`, while a later hosted add/search probe succeeded. Managed persistence is connected, but each run still records service errors rather than treating them as silent success.
